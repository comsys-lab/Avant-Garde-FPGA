// Verilator C++ harness for tb_fetch (Level 8b)
...
int main(int argc, char **argv) {
    ...
    const std::unique_ptr<Vtb_fetch> dut{new Vtb_fetch{contextp.get()}};

    const char *vcd_file = nullptr;
    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "+trace=", 7) == 0) vcd_file = argv[i] + 7;
        else if (strcmp(argv[i], "+trace") == 0) vcd_file = "wave_fetch.vcd";
    }
    ...
    // 5 TCs × ~30 cycles × 10ns + margin
    const vluint64_t MAX_TIME = 50000;
