// Verilator C++ harness for tb_issue (Level 7)
...
int main(int argc, char **argv) {
    ...
    const std::unique_ptr<Vtb_issue> dut{new Vtb_issue{contextp.get()}};

    // Parse +trace or +trace=<file> from argv
    const char *vcd_file = nullptr;
    for (int i = 1; i < argc; i++) {
        if (strncmp(argv[i], "+trace=", 7) == 0) vcd_file = argv[i] + 7;
        else if (strcmp(argv[i], "+trace") == 0) vcd_file = "wave_issue.vcd";
    }
    ...
    // MAX_TIME: 7 TCs × 16+ uops × ~30 cycles × 10ns + margin = ~120M ps
    const vluint64_t MAX_TIME = 120000000;
